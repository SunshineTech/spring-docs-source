...
当您将它们组合起来创建新 `Specification` 对象时，规范的力量真的很闪耀。您可以通过我们提供的 `Specifications` 的默认方法来实现这一点，以构建如下表达式：
...
----
`Specification` 提供了一些胶水代码默认方法来链接和组合 `Specification` 实例。这些方法让您通过创建新 `Specification` 实现并将它们与现有实现相结合来扩展数据访问层。
====
